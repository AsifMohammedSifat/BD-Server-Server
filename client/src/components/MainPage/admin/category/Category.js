...
export default function Category() {
    ...
    const createCategory = async() => {
        try {
            if(edit) {
                const response = await axios.put(`https://intense-lake-37547.herokuapp.com/api/category/${id}`, {name: category}, {
                    headers: {Authorization: token}
                })
                Swal.fire("Success!", response.data.msg, "success")
            }
            else {
                const response = await axios.post(`https://intense-lake-37547.herokuapp.com/api/category`, {name: category}, {
                    headers: {Authorization: token}
                })
                Swal.fire("Success!", response.data.msg, "success")
            }
    ...
    const deleteCategory = async(id) => {
         try {
             const response = await axios.delete(`https://intense-lake-37547.herokuapp.com/api/category/${id}`, {
                 headers: {Authorization: token}
             })
